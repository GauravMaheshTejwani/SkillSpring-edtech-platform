            ...
            <div className=' flex flex-col w-[calc(100vw-20%)] md:w-fit items-start'>
                <div className=' ml-10 relative mb-2 flex w-full justify-center'>
                    {steps.map((item) => (
                        ...
                        ? "bg-yellow-700 border-yellow-400 text-yellow-400" 
                        : "border-gray-700 bg-gray-800 text-gray-400"}`}>
                            ...
                            {item.id< 3 && (
                                <div className={`h-[calc(34px/2)] w-[100%]  border-dashed border-b-2 ${step > item.id ? "border-yellow-400" : "border-gray-700"}}`}>
                                </div>
                            )}
                        </div>
                    ))}
                ...
                <div className='relative mb-16 flex w-full select-none justify-between'>
                    {steps.map((item) => (
                            ...
                            <div key={item.id} className='flex md:min-w-[180px] flex-col items-start'>
                                <p className=' ml-3 md:ml-0 text-[10px] md:text-sm text-gray-50'>{item.title}</p>
                            </div>
